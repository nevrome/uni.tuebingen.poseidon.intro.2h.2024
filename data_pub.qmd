...
So far we have used data in the public Poseidon community archive, now it's time to learn how to give back. Poseidon is very dependent on data and time donations from the community.
...
We already built a minimal Poseidon package for our data in previous sections. We even made modifications to the `.janno` file to include more context data. 

Before we submit the package we should check more information to `POSEIDON.yml`, `.janno` and `.bib` to include the meta information we consider relevant. All of these files are well specified and documented.

Here is the current state of our `POSEIDON.yml` file:

```{yml, filename="POSEIDON.yml"}
...
And here is a `POSEIDON.yml` file from a recently submitted, real world package:
...
::: {.callout-note}
Some tasks in the package booking process for these files can be automated with [`trident rectify`](https://www.poseidon-adna.org/#/trident?id=rectify-command).
:::
...
::: {.callout-tip}
## Learn more
The process is documented in more detail in a [**Submission guide**](https://www.poseidon-adna.org/#/archive_submission_guide) on the website.
:::