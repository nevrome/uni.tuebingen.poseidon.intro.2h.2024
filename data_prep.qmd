...
::: {.callout-caution}
`trident` is also available for Windows and macOS, but security features of these operating systems complicate the installation.
:::

## Turning the data into a Poseidon package

At the moment we only have the Eigenstrat data. To incorporate context information, for example the coordinates we received from Prof. P., we will create a Poseidon package around it.

::: {.callout-note}
`trident` supports direct handling of genotype data without a Poseidon package with the `-p`/`--genoOne` flag.
:::

We can create the package with `trident`'s [`init`](https://www.poseidon-adna.org/#/trident?id=init-command) subcommand, which wraps genotype data in a Poseidon package.
